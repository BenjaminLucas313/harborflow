...
import { useVibrate, useConfirmPop } from "@/hooks/useButtonAnimation";
...
export function TripStatusManager({ tripId, currentStatus, automatizado }: Props) {
  ...
  const vibrateDelete    = useVibrate();
  const vibrateDeauto    = useVibrate();
  const popConfirm       = useConfirmPop();
    ...
    <>
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-xl space-y-5">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => { popConfirm.trigger(); void handleModalConfirm(); }}
                disabled={modalPending}
                className={`rounded-xl px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50 ${modal.confirmClass} ${popConfirm.className}`}
              >
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {automatizado && (
            <button
              onClick={() => { vibrateDeauto.trigger(); openDesautomatizarModal(); }}
              disabled={loading !== null}
              className={`rounded-xl border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 hover:bg-amber-100 transition-colors disabled:opacity-50 ${vibrateDeauto.className}`}
            >
          ...
          {isCancellable && (
            <button
              onClick={() => { vibrateDelete.trigger(); openDeleteModal(); }}
              disabled={loading !== null}
              className={`rounded-xl border border-red-300 bg-red-50 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-100 transition-colors disabled:opacity-50 ${vibrateDelete.className}`}
            >
              Eliminar viaje
            </button>
          )}
