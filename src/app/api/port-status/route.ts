...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // Best-effort — notify all active company users about the port status change.
  ...
  const estadoLabel = STATUS_LABELS[status] ?? status;
    ...
    Promise.all(
      users.map((u) =>
        crearNotificacion({
          ...
          companyId,
          tipo:      "PUERTO_ESTADO",
          titulo:    "Estado del puerto actualizado",
          mensaje:   `El puerto cambió a ${estadoLabel}${message?.trim() ? `: ${message.trim()}` : ""}`,
          accionUrl: "/",
        }).catch(() => {}),
      ),
    ),
